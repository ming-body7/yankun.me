...
final class Youxi_Instagram {
	...
	public static function get( $q, $count ) {
		...
		/* Instagram client ID */
		$client_id = apply_filters( 'youxi_instagram_client_id', '' );
		...
		if( ! is_array( $data ) || empty( $data ) ) {
			...
			if( 200 == wp_remote_retrieve_response_code( $response ) ) {
				...
				if( isset( $user_id ) ) {
					...
					return new WP_Error( 'invalid_username', sprintf( __( 'The username <strong>%s</strong> is invalid.', 'youxi' ), $q ), $response );
				}
