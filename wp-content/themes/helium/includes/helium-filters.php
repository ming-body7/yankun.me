...
/* ==========================================================================
	WordPress 4.4 work around for `redirect_canonical`
	that causes AJAX pagination to break on a static frontpage.
============================================================================= */

if( ! function_exists( 'helium_redirect_canonical' ) ):

function helium_redirect_canonical( $redirect_url ) {
	if( is_front_page() && ! is_home() && get_query_var( 'page' ) ) {
		return false;
	}
	return $redirect_url;
}
endif;
add_filter( 'redirect_canonical', 'helium_redirect_canonical' );
